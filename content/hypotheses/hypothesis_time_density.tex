...
The concrete approach to evaluate these hypotheses begins with the function $\mathit{range}$ that maps a timespan to a set of timestamps $\varT^2 \rightarrow \varP(\varT)$, where $\varP(\varT)$ is the power set of $\varT$. 
It is defined as
...
%The number of facts $\mathit{num}(f_\tau)$ in the timespans $f_\tau$ of every fact $f \in \varG$ is aggregated into an ordered sequence of numbers $S$, and the \nth{25} percentage value $p$ and the \nth{75} percentage value $d$ is found for $S$. The sparse partition on test set $T$ is then defined as

%The \nth{25} percentage value $p$ and the \nth{75} percentage value $d$ is found for $\mathit{num}(\tau)$. $\mathit{num}(\tau)$ exists for each temporal granularity $\tau$ in $\varG$. 
%$\tau$ is the 
%for all timespans $\tau$ in the facts of $\varG$. The sparse partition on test set $T$ is then defined as
...
\noindent
where $p$ is the \nth{25} percentile value of $O$. The dense partition $T_D$ is similarly defined as the set of facts $f \in T$ where the number of facts that fall within the timespan of $f_\tau$ is higher than $d$:
